...
pub fn handler(ctx: Context<AutoRelease>, _contract_id: [u8; 32]) -> Result<()> {
    ...
    require!(
        now >= ctx.accounts.escrow.approval_deadline,
        EscrowError::ApprovalDeadlineNotReached
    );

    let price = ctx.accounts.escrow.price_lamports;
    let fee = ctx.accounts.escrow.fee_lamports;

    // PDA → creator (price), PDA → treasury (fee). `close = brand` returns
    // the residual rent at end of ix.
    {
